    ...
    if ((q.type === 'multiple_choice' || q.type === 'true_false') && q.options) {
      return `
        <div style="margin-left: 15px; margin-top: 10px;">
          ${q.options.map((opt, i) => `
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px; padding: 8px 12px; background: #f8fffe; border-radius: 6px;">
              <span style="width: 22px; height: 22px; border: 2px solid #0d9488; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 600; color: #0d9488; flex-shrink: 0;">${String.fromCharCode(65 + i)}</span>
              <span style="color: #333; font-size: 13px;">${renderText(opt)}</span>
            </div>
          `).join('')}
        </div>
      `;
    ...
    <div style="font-family: 'Segoe UI', Arial, sans-serif; color: #333; padding: 20px; background: white; max-width: 800px; margin: 0 auto;">
      ...
      <div style="text-align: center; margin-bottom: 25px; padding-bottom: 15px; border-bottom: 3px solid #0d9488;">
        ...
        <div style="display: flex; justify-content: center; gap: 10px; flex-wrap: wrap;">
          <span style="background: #e0f2f1; color: #00695c; padding: 4px 12px; border-radius: 15px; font-size: 11px;">📚 ${worksheet.subject}</span>
          <span style="background: #e0f2f1; color: #00695c; padding: 4px 12px; border-radius: 15px; font-size: 11px;">📊 Grade ${worksheet.grade_level}</span>
          <span style="background: #e0f2f1; color: #00695c; padding: 4px 12px; border-radius: 15px; font-size: 11px;">⚡ ${worksheet.difficulty}</span>
          <span style="background: #e0f2f1; color: #00695c; padding: 4px 12px; border-radius: 15px; font-size: 11px;">🌐 ${worksheet.language.toUpperCase()}</span>
        </div>
      </div>
          ...
          <div style="margin-bottom: 20px; padding: 15px; background: #fafafa; border-left: 4px solid #0d9488; border-radius: 6px; page-break-inside: avoid;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
              <div style="display: flex; align-items: center; gap: 8px;">
                <span style="display: inline-block; background: #0d9488; color: white; width: 24px; height: 24px; border-radius: 50%; text-align: center; line-height: 24px; font-weight: bold; font-size: 12px;">${index + 1}</span>
              ...
              <span style="background: #0d9488; color: white; padding: 3px 10px; border-radius: 10px; font-size: 10px; font-weight: 600;">${getQuestionPoints(index)} pts</span>
            </div>
